      ...
      <main className="pb-20 md:pt-24">
        <section className="border-b border-[#F0E6E2] bg-[#F6F3EA]">
          <div className="mx-auto w-full max-w-[1500px] px-4 pb-5 pt-6 md:px-6 md:pb-7 md:pt-8">
            <div className="flex items-end justify-between gap-5">
              <div>
                <div className="mb-2 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#0E0E0E]" /><span className="font-serif text-[7px] tracking-[0.25em] text-[#D8C29A]">{getSiteText(content, "new_arrivals_eyebrow", "NEW IN")}</span></div>
              ...
              {!isLoading && total > 0 && <div className="shrink-0 text-left"><span className="block text-[18px] font-semibold leading-none text-[#D8C29A] md:text-[22px]">{total}</span><span className="mt-1 block text-[6px] text-[#929A92] md:text-[7px]">قطعة جديدة</span></div>}
            </div>
          </div>
        </section>
        ...
        <section className="border-b border-[#EFE6E2] bg-white">
          <div className="mx-auto flex h-[42px] w-full max-w-[1500px] items-center justify-between px-3 md:h-[46px] md:px-6">
            <div className="flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5 stroke-[1.5] text-[#D8C29A]" /><span className="text-[8px] font-medium text-[#755F5E] md:text-[9px]">أحدث الإضافات</span></div>
        ...
        <section className="mx-auto w-full max-w-[1500px] px-2.5 pt-4 md:px-6 md:pt-6">
          {isLoading ? (
            ...
            <div className="flex min-h-[48vh] flex-col items-center justify-center px-5 text-center">
              <div className="flex h-[64px] w-[64px] items-center justify-center rounded-full bg-[#F5F5F5]"><Sparkles className="h-5 w-5 stroke-[1.4] text-[#D8C29A]" /></div>
              <span className="mt-4 font-serif text-[6px] tracking-[0.22em] text-[#D8C29A]">GENAN</span>
            ...
            <>
              ...
              {hasMore && <div className="flex justify-center pt-8"><button type="button" disabled={isFetching} onClick={() => setVisibleCount((count) => count + PAGE_SIZE)} className="flex h-11 min-w-[150px] items-center justify-center gap-2 rounded-full border border-[#D4C9B3] bg-white px-6 text-[9px] font-semibold text-[#0E0E0E] disabled:opacity-50">{isFetching && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{isFetching ? "جارٍ التحميل" : "عرض المزيد"}</button></div>}
            </>
          )}
        </section>

        {!isLoading && products.length > 0 && <section className="mx-auto w-full max-w-[1500px] px-3 py-9 md:px-6 md:py-12"><div className="border-t border-[#EADFDA] pt-6 text-center"><div className="mb-2 flex items-center justify-center gap-2"><span className="h-px w-5 bg-[#C6B17F]" /><span className="font-serif text-[6px] tracking-[0.24em] text-[#D8C29A]">GENAN NEW EDIT</span><span className="h-px w-5 bg-[#C6B17F]" /></div><p className="text-[9px] text-[#858E86]">تتجدد اختياراتنا باستمرار لتكتشف كل جديد.</p></div></section>}
      </main>
