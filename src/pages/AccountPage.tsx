...
const AccountPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-[#302725]" dir="rtl">
      <Navbar />
      <CartDrawer />
      ...
      <main className="pb-20 md:pt-24">
        <div className="mx-auto w-full max-w-[1050px]">
          ...
          <section className="border-b border-[#E5DED0] bg-[#FFF8F6] px-4 pb-5 pt-6 md:mx-6 md:mt-6 md:rounded-[20px] md:border md:px-6 md:py-6">
            <div className="flex items-center gap-4">
              <div className="relative shrink-0">
                <div className="flex h-[70px] w-[70px] items-center justify-center overflow-hidden rounded-full border border-[#E5DED0] bg-[#FAE7E5] md:h-[82px] md:w-[82px]">
                  {customer?.avatar_url ? (
                    ...
                    <User className="h-7 w-7 stroke-[1.4] text-[#D8C29A]" />
                  )}
                </div>

                <button type="button" onClick={handleSettingsClick} className="absolute -bottom-0.5 -left-0.5 flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#FFF8F6] bg-[#0E0E0E] text-white" aria-label="تعديل الملف الشخصي">
              ...
              <button type="button" onClick={handleSettingsClick} className="hidden h-9 shrink-0 items-center gap-1.5 rounded-full border border-[#E4D6D2] bg-white px-4 text-[8px] font-medium text-[#665652] md:flex">
                <Settings className="h-3.5 w-3.5 stroke-[1.5] text-[#D8C29A]" />
                تعديل الحساب
              </button>
            </div>
            ...
            <div className="mt-5 grid grid-cols-3 divide-x divide-x-reverse divide-[#E8D8D4] border-t border-[#E9DDD9] pt-4">
              <div className="text-center">
                <span className="block text-[15px] font-semibold leading-none text-[#D8C29A]">{favorites.length}</span>
                <span className="mt-1.5 block text-[6px] text-[#9E8E89]">المفضلة</span>
              </div>

              <div className="text-center">
                <span className="block text-[15px] font-semibold leading-none text-[#D8C29A]">{invoices.length}</span>
                <span className="mt-1.5 block text-[6px] text-[#9E8E89]">الطلبات</span>
              </div>

              <div className="text-center">
                <span className="block text-[15px] font-semibold leading-none text-[#D8C29A]">{activeShipments.length}</span>
          ...
          <section className="px-3 pt-5 md:px-6 md:pt-7">
            <div className="mb-3">
              <span className="font-serif text-[6px] tracking-[0.22em] text-[#D8C29A]">QUICK ACCESS</span>
              <h2 className="mt-1 text-[15px] font-semibold text-[#0E0E0E] md:text-[18px]">حسابي</h2>
            </div>

            <div className="grid grid-cols-3 gap-2">
              {mainItems.map((item) => (
                <Link key={item.to} to={item.to} className="flex min-w-0 flex-col items-center rounded-[14px] border border-[#EBE1DD] bg-white px-2 py-4 text-center active:bg-[#FAFAFA]">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F5F5F5] text-[#C56C72]">
          ...
          <section className="px-3 pt-7 md:px-6">
            <div className="mb-3 flex items-end justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <Truck className="h-4 w-4 stroke-[1.5] text-[#D8C29A]" />
                  <h2 className="text-[15px] font-semibold text-[#0E0E0E] md:text-[18px]">شحناتي الحالية</h2>
                </div>
              ...
              {latestOrderNumber && (
                <button type="button" onClick={() => navigate(`/order-tracking?order=${encodeURIComponent(latestOrderNumber)}`)} className="text-[7px] font-medium text-[#D8C29A]">
                  تتبع آخر طلب
                </button>
              )}
            </div>
            ...
            <div className="overflow-hidden rounded-[16px] border border-[#EAEAEA] bg-white">
              ...
              {!invoicesLoading &&
                activeShipments.map((invoice, index) => {
                  ...
                  const barTone = shippingProgressBarMap[status] || "bg-[#0E0E0E]";
                  ...
                  return (
                    <div key={`shipment-${invoice.id}`} className={`p-4 ${index !== activeShipments.length - 1 ? "border-b border-[#EBE6DC]" : ""}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button type="button" onClick={() => navigate(`/order-tracking?order=${encodeURIComponent(invoice.order_number)}`)} className="flex h-8 shrink-0 items-center gap-1 rounded-full border border-[#E2D4D0] px-3 text-[7px] font-medium text-[#D8C29A]">
                          تتبع
          ...
          <section id="account-orders" className="scroll-mt-24 px-3 pt-7 md:px-6">
            <div className="mb-3 flex items-end justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <Receipt className="h-4 w-4 stroke-[1.5] text-[#D8C29A]" />
                  <h2 className="text-[15px] font-semibold text-[#0E0E0E] md:text-[18px]">سجل فواتيري</h2>
                </div>

                <p className="mt-1 text-[7px] text-[#9F918C]">آخر 20 طلبًا</p>
              </div>

              <Link to="/my-orders" className="flex items-center gap-1 text-[7px] font-medium text-[#D8C29A]">
                كل الطلبات
                <ChevronLeft className="h-3 w-3 stroke-[1.5]" />
              </Link>
            </div>
            ...
            <div className="mb-3 grid grid-cols-2 gap-2">
              <div className="rounded-[14px] border border-[#EAEAEA] bg-white px-3 py-3">
                <span className="text-[6px] text-[#A29590]">عدد الفواتير</span>
                <span className="mt-1 block text-[17px] font-semibold leading-none text-[#D8C29A]">{invoices.length}</span>
              </div>

              <div className="rounded-[14px] border border-[#EAEAEA] bg-white px-3 py-3">
                <span className="text-[6px] text-[#A29590]">إجمالي الطلبات</span>
                <span className="mt-1 block truncate text-[17px] font-semibold leading-none text-[#D8C29A]">{invoiceTotal.toLocaleString("ar-EG")}</span>
              </div>
            </div>
                  ...
                  return (
                    <div key={invoice.id} className={`flex items-center justify-between gap-3 p-4 ${index !== invoices.length - 1 ? "border-b border-[#EBE6DC]" : ""}`}>
                      <div className="min-w-0">
                        ...
                        <button type="button" onClick={() => openInvoice(invoice)} className="mt-2 rounded-full border border-[#E0D2CE] px-2.5 py-1.5 text-[6px] font-medium text-[#D8C29A]">
                          عرض الفاتورة
                        </button>
                      </div>

                      <div className="shrink-0 text-left">
                        <p className="text-[11px] font-semibold text-[#D8C29A]">{Number(invoice.total).toLocaleString("ar-EG")}</p>
          ...
          <section className="px-3 pt-7 md:px-6">
            <div className="mb-3">
              <div className="flex items-center gap-2">
                <MapPin className="h-4 w-4 stroke-[1.5] text-[#D8C29A]" />
                <h2 className="text-[15px] font-semibold text-[#0E0E0E] md:text-[18px]">العناوين المحفوظة</h2>
              </div>

              <p className="mt-1 text-[7px] text-[#9F918C]">احفظ عناوينك لتسريع عملية الطلب</p>
            </div>

            <div id="saved-address-form" className="rounded-[16px] border border-[#EAEAEA] bg-white p-3 md:p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                {editingAddressId && (
                  <button type="button" onClick={resetAddressForm} className="text-[7px] font-medium text-[#D8C29A]">
                    إلغاء التعديل
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={addressForm.label} onChange={(event) => setAddressForm((current) => ({ ...current, label: event.target.value }))} placeholder="اسم العنوان" className="h-[42px] w-full rounded-[11px] border border-[#E8DEDA] bg-[#FFFFFF] px-3 text-[9px] text-[#554744] outline-none placeholder:text-[#AFA39E] focus:border-[#EAEAEA]" />

                <input value={addressForm.city} onChange={(event) => setAddressForm((current) => ({ ...current, city: event.target.value }))} placeholder="المدينة *" className="h-[42px] w-full rounded-[11px] border border-[#E8DEDA] bg-[#FFFFFF] px-3 text-[9px] text-[#554744] outline-none placeholder:text-[#AFA39E] focus:border-[#EAEAEA]" />
              </div>

              <input value={addressForm.address} onChange={(event) => setAddressForm((current) => ({ ...current, address: event.target.value }))} placeholder="العنوان بالتفصيل *" className="mt-2 h-[42px] w-full rounded-[11px] border border-[#E8DEDA] bg-[#FFFFFF] px-3 text-[9px] text-[#554744] outline-none placeholder:text-[#AFA39E] focus:border-[#EAEAEA]" />

              <textarea value={addressForm.notes} onChange={(event) => setAddressForm((current) => ({ ...current, notes: event.target.value }))} placeholder="ملاحظات إضافية" rows={2} className="mt-2 w-full resize-none rounded-[11px] border border-[#E8DEDA] bg-[#FFFFFF] px-3 py-3 text-[9px] text-[#554744] outline-none placeholder:text-[#AFA39E] focus:border-[#EAEAEA]" />

              <button type="button" onClick={saveAddress} className="mt-2.5 h-[42px] w-full rounded-[11px] bg-[#0E0E0E] text-[9px] font-semibold text-white active:bg-[#D8C29A]">
            ...
            <div className="mt-2.5 space-y-2">
              ...
              {savedAddresses.map((address) => (
                <div key={address.id} className="rounded-[14px] border border-[#EAEAEA] bg-white p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {address.isDefault && (
                          <span className="flex shrink-0 items-center gap-1 rounded-full bg-[#FAEDEA] px-2 py-1 text-[6px] font-medium text-[#D8C29A]">
                            <Star className="h-2.5 w-2.5 fill-[#D8C29A] stroke-[#D8C29A]" />
                            افتراضي
                          </span>
                        )}
                      </div>
                  ...
                  <div className="mt-3 flex items-center gap-1.5 border-t border-[#EBE6DC] pt-2.5">
                    ...
                    <button type="button" onClick={() => deleteAddress(address.id)} className="mr-auto rounded-full border border-[#E5DED0] px-2.5 py-1.5 text-[6px] font-medium text-[#D8C29A]">
                      حذف
                    </button>
                  </div>
                </div>
          ...
          <section className="px-3 pt-7 md:px-6">
            <h2 className="mb-3 text-[15px] font-semibold text-[#0E0E0E] md:text-[18px]">أكثر خيارات</h2>

            {settingsItems.map((item) => (
              <button key={item.to} type="button" onClick={handleSettingsClick} className="flex w-full items-center gap-3 rounded-[14px] border border-[#EAEAEA] bg-white p-3.5 text-right active:bg-[#FFF8F6]">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#F5F5F5]">
                  <item.icon className="h-4 w-4 stroke-[1.5] text-[#D8C29A]" />
                </span>
          ...
          <section className="px-3 pb-10 pt-6 md:px-6">
            <button type="button" onClick={handleLogout} className="flex h-[44px] w-full items-center justify-center gap-2 rounded-[13px] border border-[#E5DED0] bg-[#FFF7F6] text-[9px] font-semibold text-[#D8C29A] active:bg-[#FCECEA]">
        ...
        <AnimatePresence>
          {editMode && (
            <>
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="fixed inset-x-0 bottom-0 z-[90] flex max-h-[92vh] flex-col rounded-t-[26px] bg-[#FFFFFF] shadow-[0_-10px_35px_rgba(50,35,30,.10)] md:bottom-auto md:left-1/2 md:right-auto md:top-1/2 md:w-[460px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[20px]" dir="rtl">
                <div className="shrink-0 border-b border-[#ECE2DE] bg-[#FFFFFF] px-4 pb-4 pt-3 md:px-5 md:pt-5">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <span className="font-serif text-[6px] tracking-[0.22em] text-[#D8C29A]">MY PROFILE</span>
                      </div>
                ...
                <form onSubmit={handleSaveProfile} className="flex-1 space-y-4 overflow-y-auto px-4 pb-[calc(env(safe-area-inset-bottom)+20px)] pt-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:px-5 md:pb-5">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="relative h-[82px] w-[82px] overflow-hidden rounded-full border border-[#E4CECA] bg-[#F5F5F5]">
                      {avatarPreview ? (
                        <img src={avatarPreview} alt="معاينة الصورة" className="h-full w-full object-cover" />
                      ) : (
                        <div className="flex h-full w-full items-center justify-center">
                          <Camera className="h-6 w-6 stroke-[1.4] text-[#D8C29A]" />
                        </div>
                      )}
                    </div>
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={formLoading} className="mt-2 flex items-center gap-1.5 text-[7px] font-medium text-[#D8C29A] disabled:opacity-50">
                  ...
                  <label className="block">
                    ...
                    <input type="text" value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder="أدخل اسمك الكامل" disabled={formLoading} className="h-[46px] w-full rounded-[13px] border border-[#E6DBD7] bg-white px-3 text-[9px] text-[#4F423E] outline-none placeholder:text-[#AA9D97] focus:border-[#EAEAEA] disabled:opacity-50" />
                  </label>
                  ...
                  <label className="block">
                    ...
                    <input type="tel" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} placeholder="أدخل رقم الهاتف" disabled={formLoading} className="h-[46px] w-full rounded-[13px] border border-[#E6DBD7] bg-white px-3 text-[9px] text-[#4F423E] outline-none placeholder:text-[#AA9D97] focus:border-[#EAEAEA] disabled:opacity-50" />
                  </label>
                  ...
                  <div>
                    ...
                    <button type="button" onClick={() => { if (!formLoading) { setRegionSearch(""); setRegionPickerOpen(true); } }} disabled={formLoading} className="flex h-[46px] w-full items-center justify-between rounded-[13px] border border-[#E6DBD7] bg-white px-3 text-right disabled:opacity-50">
                      <div className="flex min-w-0 items-center gap-2.5">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#F5F5F5]">
                          <MapPin className="h-3.5 w-3.5 stroke-[1.5] text-[#D8C29A]" />
                        </span>
                      ...
                      <ChevronDown className="h-3.5 w-3.5 shrink-0 stroke-[1.5] text-[#D8C29A]" />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-[1.4fr_.8fr] gap-2 pt-1">
                    <button type="submit" disabled={formLoading} className="flex h-[46px] items-center justify-center gap-2 rounded-[13px] bg-[#0E0E0E] text-[9px] font-semibold text-white disabled:opacity-50">
                      {formLoading ? (
                        <>
        ...
        <AnimatePresence>
          {regionPickerOpen && (
            <>
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="fixed inset-x-0 bottom-0 z-[120] flex max-h-[82vh] flex-col rounded-t-[26px] bg-[#FFFFFF] shadow-[0_-12px_35px_rgba(50,35,30,.12)] md:bottom-auto md:left-1/2 md:right-auto md:top-1/2 md:w-[420px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[20px]" dir="rtl">
                {/* HEADER */}
                ...
                <div className="shrink-0 px-4 pt-3 md:px-5 md:pt-5">
                  ...
                  <div className="flex items-start justify-between border-b border-[#E5DED0] pb-4">
                    <div>
                      <div className="mb-1 flex items-center gap-2">
                        <span className="h-[2px] w-4 rounded-full bg-[#0E0E0E]" />
                        <span className="font-serif text-[6px] tracking-[0.22em] text-[#D8C29A]">GENAN LOCATION</span>
                      </div>
                ...
                <div className="shrink-0 px-4 pt-3 md:px-5">
                  <div className="relative">
                    ...
                    <input value={regionSearch} onChange={(event) => setRegionSearch(event.target.value)} placeholder="ابحث عن المحافظة..." className="h-[43px] w-full rounded-[12px] border border-[#E8DEDA] bg-white pr-9 pl-8 text-[9px] text-[#51433F] outline-none placeholder:text-[#AEA19B] focus:border-[#EAEAEA]" />
                ...
                <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:px-5">
                  {filteredRegions.length === 0 ? (
                    ...
                    <div className="overflow-hidden rounded-[15px] border border-[#EAEAEA] bg-white">
                        ...
                        return (
                          <button key={item} type="button" onClick={() => { setRegion(item); setRegionPickerOpen(false); setRegionSearch(""); }} className={`flex h-[48px] w-full items-center justify-between px-3.5 text-right ${index !== filteredRegions.length - 1 ? "border-b border-[#EBE6DC]" : ""} ${active ? "bg-[#FFF0EE]" : "bg-white active:bg-[#FBF7F5]"}`}>
                            <div className="flex items-center gap-2.5">
                              <span className={`flex h-7 w-7 items-center justify-center rounded-full ${active ? "bg-[#F7F7F7]" : "bg-[#F8F4F2]"}`}>
                                <MapPin className={`h-3.5 w-3.5 stroke-[1.5] ${active ? "text-[#D8C29A]" : "text-[#A99B96]"}`} />
                              </span>

                              <span className={`text-[10px] font-medium ${active ? "text-[#0E0E0E]" : "text-[#51433F]"}`}>{item}</span>
                            </div>

                            {active && (
                              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#0E0E0E]">
                ...
                <div className="shrink-0 border-t border-[#EDE4E0] bg-[#FFFFFF] px-4 pb-[calc(env(safe-area-inset-bottom)+13px)] pt-3 md:px-5 md:pb-5">
                  <div className="grid grid-cols-[.75fr_1.4fr] gap-2.5">
                    ...
                    <button type="button" onClick={() => setRegionPickerOpen(false)} disabled={!region} className="flex h-[44px] items-center justify-center gap-2 rounded-[12px] bg-[#0E0E0E] text-[9px] font-semibold text-white disabled:opacity-45">
                      <Check className="h-3.5 w-3.5" />
                      تأكيد المحافظة
                    </button>
                  </div>
