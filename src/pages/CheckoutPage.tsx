...
const CheckoutPage = () => {
  ...
  if (cart.length === 0) {
    return <div className="min-h-screen bg-[#F8F6F0]" dir="rtl"><Navbar /><CartDrawer /><main className="flex min-h-[65vh] items-center justify-center px-4 py-16"><div className="text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#EAE5D7]"><ShoppingBag className="h-5 w-5 text-[#9D7B40]" strokeWidth={1.4} /></span><h1 className="mt-4 text-[18px] font-semibold text-[#173A2D]">السلة فارغة</h1><p className="mt-1.5 text-[8px] text-[#9B8D88]">أضف بعض المنتجات قبل إتمام الطلب.</p><button type="button" onClick={() => navigate("/products")} className="mt-5 h-10 rounded-[10px] bg-[#173A2D] px-6 text-[9px] font-semibold text-white">تصفح المنتجات</button></div></main><Footer /></div>;
  }
    ...
    <div className="min-h-screen bg-[#F8F6F0]" dir="rtl">
      ...
      <main className="pb-12 pt-5 md:pb-16 md:pt-7">
        <div className="mx-auto w-full max-w-[1120px] px-3 md:px-6">
          <div className="mb-5 md:mb-7"><div className="flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#173A2D]" /><span className="font-serif text-[6px] tracking-[0.22em] text-[#9D7B40]">CHECKOUT</span></div><h1 className="mt-1.5 text-[19px] font-semibold tracking-[-0.025em] text-[#3E3431] md:text-[26px]">إتمام الطلب</h1><p className="mt-1 text-[8px] text-[#9D8F8A]">بقيت خطوات بسيطة لإتمام طلبك.</p></div>
          <div className="mb-6 overflow-hidden rounded-[14px] border border-[#E2DCCE] bg-white px-2 py-4 md:px-5"><div className="relative mx-auto w-full max-w-[580px]"><div className="pointer-events-none absolute left-[12.5%] right-[12.5%] top-[16px] h-px bg-[#EAE2DF]" /><div className="pointer-events-none absolute right-[12.5%] top-[16px] h-px bg-[#D9AAA7] transition-[width] duration-300" style={{ width: `${(currentStep / (STEPS.length - 1)) * 75}%` }} /><div className="relative grid grid-cols-4">{STEPS.map((step, index) => { const Icon = step.icon; const done = index < currentStep; const active = index === currentStep; return <div key={step.key} className="flex min-w-0 flex-col items-center justify-start text-center"><button type="button" onClick={() => { if (index < currentStep) setCurrentStep(index); }} disabled={index > currentStep} className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full border transition-colors ${done ? "border-[#173A2D] bg-[#173A2D] text-white" : active ? "border-[#173A2D] bg-[#F3F0E6] text-[#9D7B40]" : "border-[#E8DEDA] bg-[#F5F3ED] text-[#B0A49F]"}`}>{done ? <Check className="h-3.5 w-3.5" strokeWidth={2} /> : <Icon className="h-3.5 w-3.5" strokeWidth={1.5} />}</button><span className={`mt-2 block w-full text-center text-[6px] font-medium leading-none md:text-[7px] ${active || done ? "text-[#685853]" : "text-[#A99B96]"}`}>{step.label}</span></div>; })}</div></div></div>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-5"><div className="min-w-0"><section className="overflow-hidden rounded-[16px] border border-[#E2DCCE] bg-white">
            {currentStep === 1 && (
              <div className="px-4 pt-4 md:px-5 md:pt-5">
                <label htmlFor="checkout-governorate" className="mb-1.5 block text-[8px] font-medium text-[#43554B]">المحافظة *</label>
                <select id="checkout-governorate" name="governorate" value={selectedRegion} onChange={(event) => selectGovernorate(event.target.value)} className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-[9px] text-[#263B31] outline-none focus:border-[#C6B17F]">
                  <option value="">اختر المحافظة</option>
                  {YEMEN_GOVERNORATES.map((governorate) => <option key={governorate} value={governorate}>{governorate}</option>)}
                </select>
                <p className="mt-1.5 text-[6px] leading-5 text-[#8A938B]">تتحدد شركات التوصيل وطرق الدفع المتاحة حسب المحافظة.</p>
              </div>
            )}
            {currentStep === 2 && (
              <div className="px-4 pt-4 md:px-5 md:pt-5">
                <label htmlFor="checkout-payment-governorate" className="mb-1.5 block text-[8px] font-medium text-[#43554B]">محافظة التوصيل *</label>
                <select id="checkout-payment-governorate" name="payment_governorate" value={selectedRegion} onChange={(event) => selectGovernorate(event.target.value)} className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-[9px] text-[#263B31] outline-none focus:border-[#C6B17F]">
                  <option value="">اختر المحافظة</option>
                  {YEMEN_GOVERNORATES.map((governorate) => <option key={governorate} value={governorate}>{governorate}</option>)}
                </select>
            ...
            {currentStep === 3 && (
              <div className="px-4 pt-4 md:px-5 md:pt-5">
                <div className="rounded-[11px] border border-[#E2DCCE] bg-[#F8F6F0] p-3">
                  <p className="text-[7px] text-[#8C7E79]">المحافظة: <span className="font-semibold text-[#30453A]">{selectedRegion}</span></p>
                  <label className="mt-3 flex cursor-pointer items-start gap-2.5">
                    <input type="checkbox" checked={termsAccepted} onChange={(event) => setTermsAccepted(event.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 accent-[#173A2D]" />
                    <span className="text-[7px] leading-5 text-[#68736B]">قرأت وأوافق على <Link to="/terms" target="_blank" className="font-semibold text-[#9D7B40] underline underline-offset-2">الشروط والأحكام</Link> و<Link to="/privacy-policy" target="_blank" className="font-semibold text-[#9D7B40] underline underline-offset-2">سياسة الخصوصية</Link> و<Link to="/returns-policy" target="_blank" className="font-semibold text-[#9D7B40] underline underline-offset-2">سياسة الإرجاع</Link>.</span>
                  </label>
                </div>
              </div>
            )}
            {currentStep === 0 && <div className="p-4 md:p-5"><div className="mb-5 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EAE5D7]"><User className="h-3.5 w-3.5 text-[#9D7B40]" strokeWidth={1.5} /></span><div><h2 className="text-[11px] font-semibold text-[#263B31]">معلوماتك الشخصية</h2><p className="mt-0.5 text-[7px] text-[#8A938B]">بيانات التواصل الخاصة بالطلب.</p></div></div><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><div><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">الاسم الكامل *</label><input value={isGuestLike ? formData.name : customer?.name || ""} onChange={(event) => { if (isGuestLike) setFormData((current) => ({ ...current, name: event.target.value })); }} disabled={!isGuestLike} placeholder="أدخل اسمك" className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-[9px] text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F] disabled:bg-[#F7F4F2] disabled:text-[#7B857D]" /></div><div><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">رقم الهاتف *</label><input value={isGuestLike ? formData.phone : customer?.phone || ""} onChange={(event) => { if (isGuestLike) setFormData((current) => ({ ...current, phone: event.target.value })); }} disabled={!isGuestLike} placeholder="رقم الهاتف" dir="ltr" inputMode="tel" className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-left text-[9px] text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F] disabled:bg-[#F7F4F2] disabled:text-[#7B857D]" /></div></div><div className="mt-4"><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">البريد الإلكتروني <span className="font-normal text-[#949C94]">اختياري</span></label><input value={formData.email} onChange={(event) => setFormData((current) => ({ ...current, email: event.target.value }))} placeholder="you@example.com" dir="ltr" inputMode="email" className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-left text-[9px] text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F]" /></div></div>}
            {currentStep === 1 && <div className="p-4 md:p-5"><div className="mb-5 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EAE5D7]"><MapPin className="h-3.5 w-3.5 text-[#9D7B40]" strokeWidth={1.5} /></span><div><h2 className="text-[11px] font-semibold text-[#263B31]">عنوان التوصيل</h2><p className="mt-0.5 text-[7px] text-[#8A938B]">أدخل المكان الذي تريد استلام الطلب فيه.</p></div></div>{savedAddresses.length > 0 && <div className="relative mb-4"><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">عنوان محفوظ</label><button type="button" onClick={() => setAddressPickerOpen((current) => !current)} className={`flex h-11 w-full items-center justify-between rounded-[10px] border bg-white px-3 text-right text-[9px] ${addressPickerOpen ? "border-[#C6B17F]" : "border-[#DED8CA]"}`}><div className="min-w-0">{selectedSavedAddress ? <><p className="truncate font-medium text-[#30453A]">{selectedSavedAddress.label}</p><p className="mt-0.5 truncate text-[7px] text-[#8A938B]">{selectedSavedAddress.city} — {selectedSavedAddress.address}</p></> : <span className="text-[#8A938B]">إضافة عنوان جديد</span>}</div><ChevronDown className={`h-3.5 w-3.5 shrink-0 text-[#899289] transition-transform ${addressPickerOpen ? "rotate-180" : ""}`} strokeWidth={1.5} /></button>{addressPickerOpen && <><button type="button" onClick={() => setAddressPickerOpen(false)} aria-label="إغلاق قائمة العناوين" className="fixed inset-0 z-[60] cursor-default" /><div className="absolute inset-x-0 top-[68px] z-[70] overflow-hidden rounded-[12px] border border-[#DDD7C8] bg-white p-1.5 shadow-[0_12px_32px_rgba(50,35,30,0.10)]"><button type="button" onClick={() => selectSavedAddress("")} className={`flex min-h-[42px] w-full items-center justify-between rounded-[8px] px-3 text-right text-[8px] ${!selectedAddressId ? "bg-[#F3F0E6] text-[#173A2D]" : "text-[#4C5E54]"}`}><span>عنوان جديد</span>{!selectedAddressId && <Check className="h-3.5 w-3.5" />}</button>{savedAddresses.map((address) => { const active = selectedAddressId === address.id; return <button key={address.id} type="button" onClick={() => selectSavedAddress(address.id)} className={`flex min-h-[50px] w-full items-center justify-between gap-3 rounded-[8px] px-3 text-right ${active ? "bg-[#F3F0E6]" : "active:bg-[#F5F3ED]"}`}><div className="min-w-0"><p className={`truncate text-[8px] font-medium ${active ? "text-[#173A2D]" : "text-[#30453A]"}`}>{address.label}</p><p className="mt-1 truncate text-[6px] text-[#8A938B]">{address.city} — {address.address}</p></div>{active && <Check className="h-3.5 w-3.5 shrink-0 text-[#9D7B40]" />}</button>; })}</div></>}</div>}<div className="grid grid-cols-1 gap-4 md:grid-cols-2"><div><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">المدينة *</label><input value={formData.city} onChange={(event) => { setSelectedAddressId(""); setFormData((current) => ({ ...current, city: event.target.value })); }} placeholder="مثال: المنصورة أو المعلا" className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-[9px] text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F]" /></div><div><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">العنوان بالتفصيل *</label><input value={formData.address} onChange={(event) => { setSelectedAddressId(""); setFormData((current) => ({ ...current, address: event.target.value })); }} placeholder="الحي، الشارع، رقم المبنى" className="h-11 w-full rounded-[10px] border border-[#DED8CA] bg-white px-3 text-[9px] text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F]" /></div></div><div className="mt-4"><label className="mb-1.5 block text-[8px] font-medium text-[#43554B]">ملاحظات إضافية <span className="font-normal text-[#949C94]">اختياري</span></label><textarea value={formData.notes} onChange={(event) => setFormData((current) => ({ ...current, notes: event.target.value }))} rows={3} placeholder="وقت التسليم المفضل، معلم قريب..." className="w-full resize-none rounded-[10px] border border-[#DED8CA] bg-white px-3 py-3 text-[9px] leading-6 text-[#263B31] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F]" /></div></div>}
            {currentStep === 2 && <div className="p-4 md:p-5"><div className="mb-5 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EAE5D7]"><CreditCard className="h-3.5 w-3.5 text-[#9D7B40]" strokeWidth={1.5} /></span><div><h2 className="text-[11px] font-semibold text-[#263B31]">الشحن والدفع</h2><p className="mt-0.5 text-[7px] text-[#8A938B]">اختر نطاق التوصيل ثم شركة التوصيل وطريقة الدفع.</p></div></div><div><div className="mb-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => { setDeliveryScope("aden"); setSelectedDelivery(adenDeliveryCompany?.id || ""); }} className={`min-h-[42px] rounded-[10px] border px-3 text-[8px] font-semibold transition-colors ${deliveryScope === "aden" ? "border-[#173A2D] bg-[#F3F0E6] text-[#173A2D]" : "border-[#DED8CA] bg-white text-[#4C5E54]"}`}>توصيل في عدن</button><button type="button" onClick={() => { setDeliveryScope("outside"); setSelectedDelivery(""); setPaymentMethod((current) => current === "cod" || current === "cash" ? "" : current); }} className={`min-h-[42px] rounded-[10px] border px-3 text-[8px] font-semibold transition-colors ${deliveryScope === "outside" ? "border-[#173A2D] bg-[#F3F0E6] text-[#173A2D]" : "border-[#DED8CA] bg-white text-[#4C5E54]"}`}>توصيل خارج عدن</button></div><div className="mb-2.5 flex items-center gap-1.5"><Truck className="h-3.5 w-3.5 text-[#A76A6D]" strokeWidth={1.5} /><span className="text-[8px] font-semibold text-[#3F5147]">شركة التوصيل *</span></div>{visibleDeliveryCompanies.length > 0 ? <div className="grid grid-cols-1 gap-2 md:grid-cols-2">{visibleDeliveryCompanies.map((company) => { const active = selectedDelivery === company.id; return <button key={company.id} type="button" onClick={() => setSelectedDelivery(company.id)} className={`relative flex min-h-[68px] items-center justify-between rounded-[11px] border p-3 text-right ${active ? "border-[#C6B17F] bg-[#F5F1E7]" : "border-[#DED8CA] bg-white active:bg-[#F5F3ED]"}`}><div><p className={`text-[9px] font-semibold ${active ? "text-[#173A2D]" : "text-[#30453A]"}`}>{company.name}</p><p className="mt-1 text-[7px] text-[#8A938B]">{formatCurrency(company.base_fee)}{company.delivery_days ? ` • ${company.delivery_days}` : ""}</p></div><span className={`flex h-5 w-5 items-center justify-center rounded-full border ${active ? "border-[#173A2D] bg-[#173A2D]" : "border-[#D4CCBC]"}`}>{active && <Check className="h-2.5 w-2.5 text-white" strokeWidth={2.2} />}</span></button>; })}</div> : <div className="flex items-center gap-2 rounded-[10px] bg-[#F3F0E8] px-3 py-3"><AlertCircle className="h-3.5 w-3.5 text-[#899289]" /><span className="text-[7px] text-[#7B857D]">لا توجد شركات توصيل متاحة لهذا الخيار حالياً.</span></div>}</div><div className="mt-5"><p className="mb-2.5 text-[8px] font-semibold text-[#3F5147]">طريقة الدفع *</p>{visiblePaymentMethods.length > 0 ? <div className="grid grid-cols-2 gap-2">{visiblePaymentMethods.map((method) => { const active = paymentMethod === method.code; const Icon = method.type === "cash" ? Banknote : CreditCard; return <button key={method.id} type="button" onClick={() => setPaymentMethod(method.code)} className={`relative flex min-h-[72px] flex-col items-start justify-center rounded-[11px] border p-3 text-right ${active ? "border-[#C6B17F] bg-[#F5F1E7]" : "border-[#DED8CA] bg-white"}`}><Icon className={`h-4 w-4 ${active ? "text-[#9D7B40]" : "text-[#7B857D]"}`} strokeWidth={1.5} /><p className="mt-2 text-[8px] font-semibold text-[#30453A]">{method.name_ar || method.name}</p><p className="mt-0.5 text-[6px] text-[#8A938B]">{method.name}</p>{active && <span className="absolute left-2.5 top-2.5 flex h-4 w-4 items-center justify-center rounded-full bg-[#173A2D]"><Check className="h-2.5 w-2.5 text-white" /></span>}</button>; })}</div> : <div className="rounded-[10px] bg-[#F3F0E8] px-3 py-3 text-[7px] text-[#7B857D]">لا توجد طرق دفع مفعلة من لوحة الإدارة.</div>}</div>{isBankPayment && bankAccounts.length > 0 && <div className="mt-5 rounded-[12px] border border-[#E2DCCE] bg-[#F8F6F0] p-3"><p className="mb-2.5 text-[7px] text-[#858E86]">حول المبلغ إلى أحد الحسابات التالية:</p><div className="space-y-2">{bankAccounts.map((account, index) => <div key={`${account.account}-${index}`} className="flex items-center justify-between gap-3 rounded-[9px] border border-[#E6E0D4] bg-white p-3"><div className="min-w-0"><p className="truncate text-[8px] font-semibold text-[#30453A]">{account.bank}</p>{account.name && <p className="mt-0.5 truncate text-[6px] text-[#8A938B]">{account.name}</p>}<p dir="ltr" className="mt-1 truncate text-left font-mono text-[7px] text-[#6D786F]">{account.account}</p></div><button type="button" onClick={() => { navigator.clipboard.writeText(account.account); toast({ title: "تم نسخ رقم الحساب" }); }} aria-label="نسخ رقم الحساب" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] border border-[#DDD7C8] bg-white text-[#A76A6D] active:bg-[#F3F0E6]"><Copy className="h-3.5 w-3.5" strokeWidth={1.5} /></button></div>)}</div></div>}<div className="mt-5 border-t border-[#E5DED0] pt-4"><div className="mb-2 flex items-center gap-1.5"><Ticket className="h-3.5 w-3.5 text-[#A76A6D]" strokeWidth={1.5} /><span className="text-[8px] font-semibold text-[#3F5147]">كود الخصم</span><span className="text-[6px] text-[#8A938B]">اختياري</span></div>{appliedCoupon ? <div className="flex min-h-[46px] items-center justify-between gap-3 rounded-[10px] border border-[#CFE1D2] bg-[#F5FAF6] px-3"><div className="flex min-w-0 items-center gap-2"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#6F9576]"><Check className="h-3 w-3 text-white" /></span><div className="min-w-0"><p className="truncate text-[8px] font-semibold text-[#54745A]">{appliedCoupon}</p><p className="mt-0.5 text-[6px] text-[#77907B]">خصم {formatCurrency(discountAmount)}</p></div></div><button type="button" onClick={removeCoupon} className="flex h-7 items-center gap-1 rounded-[7px] px-2 text-[7px] font-medium text-[#A45D5D] active:bg-white"><X className="h-3 w-3" />إزالة</button></div> : <div className="flex gap-2"><input value={couponCode} onChange={(event) => setCouponCode(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void applyCoupon(); }} placeholder="أدخل الكود" dir="ltr" className="h-10 min-w-0 flex-1 rounded-[9px] border border-[#DED8CA] bg-white px-3 text-left text-[8px] uppercase text-[#30453A] outline-none placeholder:text-[#9AA19A] focus:border-[#C6B17F]" /><button type="button" onClick={applyCoupon} className="h-10 shrink-0 rounded-[9px] border border-[#C6B17F] bg-white px-4 text-[8px] font-semibold text-[#173A2D] active:bg-[#F5F1E7]">تطبيق</button></div>}</div></div>}
            {currentStep === 3 && <div className="p-4 md:p-5"><div className="mb-5 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EAE5D7]"><Check className="h-3.5 w-3.5 text-[#9D7B40]" strokeWidth={1.7} /></span><div><h2 className="text-[11px] font-semibold text-[#263B31]">مراجعة الطلب</h2><p className="mt-0.5 text-[7px] text-[#8A938B]">راجع معلوماتك قبل التأكيد النهائي.</p></div></div><div className="border-b border-[#E5DED0] py-3 first:pt-0"><p className="text-[7px] font-medium text-[#8A938B]">معلومات التواصل</p><p className="mt-1.5 text-[9px] font-medium text-[#30453A]">{customer?.name || formData.name}</p><p dir="ltr" className="mt-1 text-right text-[7px] text-[#758078]">{customer?.phone || formData.phone}</p></div><div className="border-b border-[#E5DED0] py-3"><p className="text-[7px] font-medium text-[#8A938B]">عنوان التوصيل</p><p className="mt-1.5 text-[9px] leading-5 text-[#30453A]">{formData.city} — {formData.address}</p>{formData.notes && <p className="mt-1 text-[7px] leading-5 text-[#858E86]">ملاحظات: {formData.notes}</p>}</div><div className="border-b border-[#E5DED0] py-3"><p className="text-[7px] font-medium text-[#8A938B]">الشحن والدفع</p><p className="mt-1.5 text-[9px] text-[#30453A]">{selectedCompany?.name || "—"} <span className="mx-1 text-[#C9BDB8]">•</span> {selectedPaymentMethod?.name_ar || selectedPaymentMethod?.name || "—"}</p>{appliedCoupon && <p className="mt-1.5 text-[7px] font-medium text-[#5C8063]">كوبون {appliedCoupon} — خصم {formatCurrency(discountAmount)}</p>}</div><div className="pt-3"><p className="mb-3 text-[7px] font-medium text-[#8A938B]">المنتجات</p><div className="space-y-3">{cart.map((item, index) => { const price = item.product.discount ? item.product.price * (1 - item.product.discount / 100) : item.product.price; const accessoriesTotal = item.selectedAccessories?.reduce((sum, accessory) => sum + accessory.price * accessory.quantity, 0) || 0; const color = item.selectedColor || item.variantColor; return <div key={`${item.product.id}-${index}`} className="flex items-center gap-3"><div className="flex h-[60px] w-[60px] shrink-0 items-center justify-center overflow-hidden rounded-[9px] border border-[#E7E1D5] bg-[#F3F0E8] p-1"><img src={optimizeImage(item.product.images?.[0] || "/placeholder.svg", 200, 80)} alt={item.product.nameAr || ""} loading="lazy" decoding="async" onError={handleImageError} className="h-full w-full object-contain object-center" /></div><div className="min-w-0 flex-1"><p className="truncate text-[8px] font-medium text-[#30453A]">{item.product.nameAr}</p><div className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-[6px] text-[#858E86]"><span>الكمية: {item.quantity}</span>{item.selectedSize && <span>المقاس: {item.selectedSize}</span>}{color && <span>اللون: {color}</span>}</div></div><span className="shrink-0 text-[8px] font-semibold text-[#173A2D]">{formatCurrency((price + accessoriesTotal) * item.quantity)}</span></div>; })}</div></div></div>}
          </section><div className="mt-4 flex items-center justify-between gap-2"><button type="button" onClick={prevStep} disabled={currentStep === 0} className="flex h-11 min-w-[88px] items-center justify-center gap-1.5 rounded-[10px] border border-[#DED8CA] bg-white px-4 text-[8px] font-semibold text-[#4C5E54] disabled:cursor-not-allowed disabled:opacity-35"><ChevronRight className="h-3.5 w-3.5" strokeWidth={1.5} />السابق</button>{currentStep < STEPS.length - 1 ? <button type="button" onClick={nextStep} className="flex h-11 min-w-[138px] items-center justify-center gap-1.5 rounded-[10px] bg-[#173A2D] px-5 text-[8px] font-semibold text-white active:bg-[#214C3B]">التالي<ChevronLeft className="h-3.5 w-3.5" strokeWidth={1.5} /></button> : <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="flex h-11 min-w-[155px] items-center justify-center gap-2 rounded-[10px] bg-[#173A2D] px-5 text-[8px] font-semibold text-white active:bg-[#214C3B] disabled:cursor-not-allowed disabled:opacity-50">{isSubmitting && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{isSubmitting ? "جارٍ إنشاء الطلب" : "تأكيد الطلب النهائي"}</button>}</div></div>
          <aside className="min-w-0 lg:sticky lg:top-24 lg:self-start"><div className="overflow-hidden rounded-[16px] border border-[#E2DCCE] bg-white"><div className="border-b border-[#E5DED0] px-4 py-3.5"><div className="flex items-center justify-between"><h2 className="text-[10px] font-semibold text-[#263B31]">ملخص الطلب</h2><span className="text-[6px] text-[#8A938B]">{cart.length} {cart.length === 1 ? "منتج" : "منتجات"}</span></div></div><div className="max-h-[280px] overflow-y-auto px-4 [scrollbar-width:thin]">{cart.map((item, index) => { const price = item.product.discount ? item.product.price * (1 - item.product.discount / 100) : item.product.price; const accessoriesTotal = item.selectedAccessories?.reduce((sum, accessory) => sum + accessory.price * accessory.quantity, 0) || 0; const color = item.selectedColor || item.variantColor; return <div key={`${item.product.id}-${index}`} className={`flex items-center gap-3 py-3 ${index !== cart.length - 1 ? "border-b border-[#EBE6DC]" : ""}`}><div className="flex h-[60px] w-[60px] shrink-0 items-center justify-center overflow-hidden rounded-[9px] border border-[#E7E1D5] bg-[#F3F0E8] p-1"><img src={optimizeImage(item.product.images?.[0] || "/placeholder.svg", 200, 80)} alt={item.product.nameAr || ""} loading="lazy" decoding="async" onError={handleImageError} className="h-full w-full object-contain object-center" /></div><div className="min-w-0 flex-1"><p className="truncate text-[7.5px] font-medium leading-5 text-[#30453A]">{item.product.nameAr}</p><div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[6px] text-[#899289]"><span>الكمية {item.quantity}</span>{item.selectedSize && <><span className="text-[#CFC7B8]">•</span><span>{item.selectedSize}</span></>}{color && <><span className="text-[#CFC7B8]">•</span><span className="max-w-[60px] truncate">{color}</span></>}</div></div><span className="shrink-0 text-[7px] font-semibold text-[#173A2D]">{formatCurrency((price + accessoriesTotal) * item.quantity)}</span></div>; })}</div><div className="border-t border-[#E5DED0] px-4 py-4"><div className="space-y-2.5"><div className="flex items-center justify-between text-[7px] text-[#68736B]"><span>المجموع الفرعي</span><span>{formatCurrency(subtotal)}</span></div><div className="flex items-center justify-between text-[7px] text-[#68736B]"><span>رسوم التوصيل</span><span>{deliveryFee > 0 ? formatCurrency(deliveryFee) : "—"}</span></div>{discountAmount > 0 && <div className="flex items-center justify-between text-[7px] font-medium text-[#63806A]"><span>{appliedCoupon ? "خصم الكود" : "خصم إطلاق جنان 10%"}</span><span>-{formatCurrency(discountAmount)}</span></div>}</div><div className="mt-4 flex items-end justify-between border-t border-[#E5DED0] pt-3"><div><span className="block text-[7px] text-[#7B857D]">الإجمالي</span><span className="mt-0.5 block text-[6px] text-[#9CA39C]">شامل رسوم التوصيل</span></div><span className="text-[15px] font-bold text-[#9D7B40]">{formatCurrency(total)}</span></div></div></div><div className="mt-2 flex items-center justify-center gap-2 py-2 text-[6px] text-[#8A938B]"><Check className="h-3 w-3 text-[#6F9275]" strokeWidth={1.7} />بيانات طلبك تُرسل بشكل آمن</div></aside></div>
        </div>
      </main>
      <Footer />
    </div>
