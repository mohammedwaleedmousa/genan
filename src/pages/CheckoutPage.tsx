...
const CheckoutPage = () => {
  ...
  if (cart.length === 0) {
    return <div className="min-h-screen bg-[#FFFFFF]" dir="rtl"><Navbar /><CartDrawer /><main className="flex min-h-[65vh] items-center justify-center px-4 py-16"><div className="text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#F5F5F5]"><ShoppingBag className="h-5 w-5 text-[#D8C29A]" strokeWidth={1.4} /></span><h1 className="mt-4 text-[18px] font-semibold text-[#0E0E0E]">السلة فارغة</h1><p className="mt-1.5 text-[8px] text-[#9B8D88]">أضف بعض المنتجات قبل إتمام الطلب.</p><button type="button" onClick={() => navigate("/products")} className="mt-5 h-10 rounded-[10px] bg-[#0E0E0E] px-6 text-[9px] font-semibold text-white">تصفح المنتجات</button></div></main><Footer /></div>;
  }
    ...
    <div className="min-h-screen bg-[#FFFFFF]" dir="rtl">
      ...
      <main className="pb-12 pt-5 md:pb-16 md:pt-7">
        <div className="mx-auto w-full max-w-[1120px] px-3 md:px-6">
          <div className="mb-5 md:mb-7"><div className="flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#0E0E0E]" /><span className="font-serif text-[6px] tracking-[0.22em] text-[#D8C29A]">CHECKOUT</span></div><h1 className="mt-1.5 text-[19px] font-semibold tracking-[-0.025em] text-[#3E3431] md:text-[26px]">إتمام الطلب</h1><p className="mt-1 text-[8px] text-[#9D8F8A]">بقيت خطوات بسيطة لإتمام طلبك.</p></div>
          <div className="mb-6 overflow-hidden rounded-[14px] border border-[#EAEAEA] bg-white px-2 py-4 md:px-5"><div className="relative mx-auto w-full max-w-[580px]"><div className="pointer-events-none absolute left-[12.5%] right-[12.5%] top-[16px] h-px bg-[#EAE2DF]" /><div className="pointer-events-none absolute right-[12.5%] top-[16px] h-px bg-[#D9AAA7] transition-[width] duration-300" style={{ width: `${(currentStep / (STEPS.length - 1)) * 75}%` }} /><div className="relative grid grid-cols-4">{STEPS.map((step, index) => { const Icon = step.icon; const done = index < currentStep; const active = index === currentStep; return <div key={step.key} className="flex min-w-0 flex-col items-center justify-start text-center"><button type="button" onClick={() => { if (index < currentStep) setCurrentStep(index); }} disabled={index > currentStep} className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full border transition-colors ${done ? "border-[#0E0E0E] bg-[#0E0E0E] text-white" : active ? "border-[#0E0E0E] bg-[#F3F0E6] text-[#D8C29A]" : "border-[#E8DEDA] bg-[#F5F3ED] text-[#B0A49F]"}`}>{done ? <Check className="h-3.5 w-3.5" strokeWidth={2} /> : <Icon className="h-3.5 w-3.5" strokeWidth={1.5} />}</button><span className={`mt-2 block w-full text-center text-[6px] font-medium leading-none md:text-[7px] ${active || done ? "text-[#685853]" : "text-[#A99B96]"}`}>{step.label}</span></div>; })}</div></div></div>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-5"><div className="min-w-0"><section className="overflow-hidden rounded-[16px] border border-[#EAEAEA] bg-white">
            {currentStep === 1 && (
            ...
            {currentStep === 3 && (
              <div className="px-4 pt-4 md:px-5 md:pt-5">
                <div className="rounded-[11px] border border-[#EAEAEA] bg-[#FFFFFF] p-3">
