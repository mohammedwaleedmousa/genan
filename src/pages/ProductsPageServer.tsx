...
const ProductsPageServer = () => {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F3F0E8] text-[#173A2D]">
      <Navbar />
      <CartDrawer />

      <main className="pb-24 md:pb-24">
        <section className="border-b border-[#173A2D]/15 bg-[#F3F0E8]">
          <div className="mx-auto grid max-w-[1760px] gap-8 px-5 pb-10 pt-14 text-right sm:px-8 md:grid-cols-[1.2fr_.8fr] md:items-end md:px-[6vw] md:pb-14 md:pt-20">
            <div className="mb-3 flex items-center gap-3">
              <span className="h-px w-12 bg-[#9D7B40]/55" />
              <span className="text-[8px] font-semibold tracking-[0.38em] text-[#9D7B40]">GENAN / CATALOG</span>
            ...
            <p className="mt-4 max-w-[540px] text-[11px] leading-7 text-[#637269] md:text-[13px]">اختيارات مرتبة كمعرض مفتوح؛ تصفّح أقل، وشاهد القطعة بوضوح أكبر.</p>
          </div>

          <div className="mx-auto max-w-[1760px] border-t border-[#173A2D]/12 px-5 py-0 sm:px-8 md:px-[6vw]">
            <div className="flex gap-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <button onClick={() => setParam("category", null)} className={`shrink-0 border-l border-[#173A2D]/12 px-5 py-4 text-[9px] font-semibold tracking-[0.02em] ${!categorySlug ? "bg-[#173A2D] text-white" : "bg-transparent text-[#637269] hover:bg-[#E9E4D8]"}`}>الكل</button>
              {categories.filter((category) => !category.parent_id).map((category) => (
                <button key={category.id} onClick={() => setParam("category", category.slug)} className={`shrink-0 border-l border-[#173A2D]/12 px-5 py-4 text-[9px] font-semibold tracking-[0.02em] ${categorySlug === category.slug ? "bg-[#173A2D] text-white" : "bg-transparent text-[#637269] hover:bg-[#E9E4D8]"}`}>{category.name_ar}</button>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-[1760px] px-3 pt-6 sm:px-5 md:px-[6vw] md:pt-10">
          {loadingProducts && products.length === 0 ? (
            ...
            <div className="flex min-h-[52vh] flex-col items-center justify-center text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#EAE5D7]"><Heart className="h-6 w-6 text-[#9D7B40]" /></div>
              <h3 className="mt-4 text-base font-semibold">لا توجد منتجات مطابقة</h3>
              <button onClick={clearFilters} className="mt-4 border border-[#173A2D]/20 bg-transparent px-6 py-2.5 text-[11px]">إعادة تعيين</button>
            </div>
          ) : (
          ...
          {hasMore && (
            <div className="flex justify-center py-10">
              <button disabled={loadingProducts} onClick={() => setLoadedPages((value) => value + 1)} className="min-w-[178px] border border-[#173A2D]/20 bg-transparent hover:bg-[#173A2D] hover:text-white px-7 py-3 text-[11px] font-medium disabled:opacity-50">
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-[100] bg-black/25" onClick={() => setFiltersOpen(false)}>
          <aside onClick={(event) => event.stopPropagation()} className="absolute inset-y-0 right-0 w-full max-w-md overflow-y-auto bg-[#F3F0E8] p-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#D8D1C3] pb-4">
              <div><h2 className="font-semibold">فلترة المنتجات</h2><p className="mt-1 text-[10px] text-[#758078]">{facetsLoading ? "جاري الحساب..." : `${draftCount} منتج مطابق`}</p></div>
            ...
            <div className="space-y-6 py-5">
              ...
              <div className="grid grid-cols-2 gap-2"><button onClick={() => setDraftParam("sale", draftSale ? null : "1")} className={`rounded-xl border p-3 text-[10px] ${draftSale ? "border-[#173A2D] bg-[#EAE5D7]" : "border-[#D3CCBC]"}`}>العروض فقط</button><button onClick={() => setDraftParam("stock", draftStock ? null : "1")} className={`rounded-xl border p-3 text-[10px] ${draftStock ? "border-[#173A2D] bg-[#EAE5D7]" : "border-[#D3CCBC]"}`}>المتوفر فقط</button></div>
            </div>

            <div className="sticky bottom-0 flex gap-2 bg-[#F3F0E8] py-4"><button onClick={() => { const next = new URLSearchParams(); if (categorySlug) next.set("category", categorySlug); if (searchQuery) next.set("search", searchQuery); setDraft(next); }} className="flex-1 rounded-xl border border-[#E1D5D0] py-3 text-[11px]">مسح</button><button onClick={applyFilters} className="flex-[2] rounded-xl bg-[#173A2D] py-3 text-[11px] font-semibold text-white">عرض {facetsLoading ? "..." : draftCount} منتج</button></div>
          </aside>
        </div>
      )}
      ...
      {sortOpen && (
        <div className="fixed inset-0 z-[110] flex items-end bg-black/25" onClick={() => setSortOpen(false)}>
          <div onClick={(event) => event.stopPropagation()} className="w-full rounded-t-3xl bg-[#F3F0E8] p-5 md:mx-auto md:mb-8 md:max-w-md md:rounded-3xl">
            ...
            {[{ value: "new", label: "الأحدث" }, { value: "best", label: "الأكثر مبيعًا" }, { value: "featured", label: "مختارة" }, { value: "price-asc", label: "الأقل سعرًا" }, { value: "price-desc", label: "الأعلى سعرًا" }].map((option) => <button key={option.value} onClick={() => { setSortOpen(false); setParam("sort", option.value); }} className={`mb-2 w-full rounded-xl border p-3 text-right text-[11px] ${sortBy === option.value ? "border-[#173A2D] bg-[#EAE5D7]" : "border-[#E7DDD9] bg-white"}`}>{option.label}</button>)}
          </div>
        </div>
      )}
    </div>
