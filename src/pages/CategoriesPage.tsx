...
const CategoriesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDFC] text-[#302725]" dir="rtl"><Navbar /><CartDrawer /><main className="pb-20 md:pt-24">
      {!selectedParent && <section className="border-b border-[#F0E7E3] bg-[#F6F3EA]"><div className="mx-auto w-full max-w-[1500px] px-4 pb-5 pt-6 md:px-6 md:pb-8 md:pt-9"><div><div className="mb-2 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#173A2D]" /><span className="font-serif text-[7px] tracking-[0.24em] text-[#9D7B40]">{getSiteText(content, "categories_page_eyebrow", "GENAN")}</span></div><h1 className="text-[26px] font-semibold leading-tight tracking-[-0.04em] text-[#173A2D] md:text-[38px]">{getSiteText(content, "categories_page_title", "تسوّقي حسب القسم")}</h1><p className="mt-1.5 max-w-[280px] text-[9px] leading-5 text-[#899289] md:max-w-md md:text-[11px]">{getSiteText(content, "categories_page_subtitle", "اكتشفي ما يناسبك من مجموعات جنان المختارة بعناية")}</p></div></div></section>}
      {selectedParent && <section className="border-b border-[#F0E7E3] bg-[#FFFDFC]"><div className="mx-auto flex h-[46px] w-full max-w-[1500px] items-center justify-between gap-3 px-3 md:h-[50px] md:px-6"><div className="flex min-w-0 items-center gap-1.5 overflow-hidden text-[9px] text-[#8A938B] md:text-[10px]"><button type="button" onClick={() => setStepParams({ parent: null, sub: null, brand: null }, { restoreKey: "root" })} className="shrink-0 transition-colors hover:text-[#9D7B40]">الأقسام</button><ChevronLeft className="h-3 w-3 shrink-0 stroke-[1.4] text-[#C9BBB6]" />{selectedSub ? <><button type="button" onClick={() => setStepParams({ sub: null, brand: null }, { restoreKey: `parent:${selectedParent.slug}` })} className="max-w-[100px] truncate text-[#756763] transition-colors hover:text-[#9D7B40] md:max-w-[180px]">{selectedParent.name_ar}</button><ChevronLeft className="h-3 w-3 shrink-0 stroke-[1.4] text-[#C9BBB6]" /><span className="max-w-[110px] truncate font-semibold text-[#473A38] md:max-w-[200px]">{selectedSub.name_ar}</span></> : <span className="max-w-[150px] truncate font-semibold text-[#473A38]">{selectedParent.name_ar}</span>}</div>{!!activeProductCategory && availableBrands.length > 0 && <div className="relative shrink-0">{brandOpen && <div className="absolute left-0 top-10 z-50 w-48 overflow-hidden rounded-[14px] border border-[#E2DCCE] bg-white shadow-[0_12px_30px_rgba(60,40,35,.09)]"><div className="max-h-56 overflow-y-auto p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button type="button" onClick={() => { setStepParams({ brand: null }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === "all" ? "bg-[#EAE5D7] font-semibold text-[#9D7B40]" : "text-[#6B5D59]"}`}>كل الماركات</button>{availableBrands.map((brand) => <button key={brand} type="button" onClick={() => { setStepParams({ brand }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === brand ? "bg-[#EAE5D7] font-semibold text-[#9D7B40]" : "text-[#6B5D59]"}`}>{brand}</button>)}</div></div>}</div>}</div></section>}
      {!selectedParent && <section className="mx-auto w-full max-w-[1500px] px-2 pb-8 pt-3 md:px-6 md:pb-12 md:pt-6">{categoriesLoading ? <div className="grid grid-cols-4 gap-1.5 md:grid-cols-3 md:gap-5">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="aspect-[4/5] animate-pulse rounded-[12px] bg-[#EEEAE1] md:rounded-[17px]" />)}</div> : <div className="grid grid-cols-4 gap-1.5 md:grid-cols-3 md:gap-5">{parents.map((category, index) => <Link key={category.id} to={`/categories?parent=${category.slug}`} onClick={() => rememberScroll("root")} className="group relative aspect-[4/5] overflow-hidden rounded-[12px] bg-[#F4F1EF] md:rounded-[20px]"><img src={optimizeImage(category.image_url || FALLBACK[category.slug] || FALLBACK.women, 960, 78)} srcSet={createImageSrcSet(category.image_url || FALLBACK[category.slug] || FALLBACK.women, [180, 360, 720], 78)} sizes="(max-width: 767px) 25vw, 33vw" alt={category.name_ar} loading={index < 4 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" width={960} height={1200} onError={handleImageError} className="h-full w-full object-cover md:transition-transform md:duration-300 md:group-hover:scale-[1.02]" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-transparent to-transparent" /><div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-0.5 p-1.5 md:gap-2 md:p-4"><div className="min-w-0">{category.name && <p className="mb-0.5 truncate text-[4px] tracking-[0.04em] text-white/65 md:text-[8px] md:tracking-[0.11em]">{category.name}</p>}<h2 className="truncate text-[10px] font-semibold text-white sm:text-[11px] md:text-[22px]">{category.name_ar}</h2></div><span className="hidden h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white text-[#9D7B40] sm:flex md:h-8 md:w-8"><ChevronLeft className="h-2.5 w-2.5 stroke-[1.7] md:h-3.5 md:w-3.5" /></span></div></Link>)}</div>}</section>}
      {selectedParent && !selectedSub && subCategories.length > 0 && <><section className="mx-auto w-full max-w-[1500px] px-4 pb-3 pt-5 md:px-6 md:pb-5 md:pt-7"><div className="flex items-end justify-between"><div><div className="mb-1.5 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#173A2D]" /><span className="font-serif text-[6px] tracking-[0.22em] text-[#9D7B40]">GENAN</span></div><h1 className="text-[22px] font-semibold tracking-[-0.035em] text-[#173A2D] md:text-[30px]">{selectedParent.name_ar}</h1><p className="mt-1 text-[8px] text-[#899289] md:text-[10px]">اختاري المجموعة التي تريدين استكشافها</p></div><span className="text-[8px] font-medium text-[#9D7B40]">{subCategories.length} أقسام</span></div></section><section className="mx-auto w-full max-w-[1500px] px-2.5 pb-9 md:px-6 md:pb-12"><div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4 md:gap-5">{subCategories.map((category, index) => <Link key={category.id} to={`/categories?parent=${selectedParent.slug}&sub=${category.slug}`} onClick={() => rememberScroll(`parent:${selectedParent.slug}`)} className="group overflow-hidden rounded-[16px] border border-[#E5DED0] bg-white"><div className="aspect-square overflow-hidden bg-[#F4F1EF]"><img src={optimizeImage(category.image_url || FALLBACK[category.slug] || FALLBACK.women, 640, 78)} srcSet={createImageSrcSet(category.image_url || FALLBACK[category.slug] || FALLBACK.women, [240, 400, 640], 78)} sizes="(max-width: 639px) 50vw, (max-width: 767px) 33vw, 25vw" alt={category.name_ar} loading={index < 2 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" width={640} height={640} onError={handleImageError} className="h-full w-full object-cover md:transition-transform md:duration-300 md:group-hover:scale-[1.02]" /></div><div className="flex min-h-[52px] items-center justify-between gap-2 px-3 py-2.5"><div className="min-w-0"><h2 className="truncate text-[11px] font-semibold text-[#263B31] md:text-[13px]">{category.name_ar}</h2>{category.name && <p className="mt-0.5 truncate text-[7px] text-[#8A938B] md:text-[8px]">{category.name}</p>}</div><ChevronLeft className="h-3.5 w-3.5 shrink-0 stroke-[1.5] text-[#9D7B40]" /></div></Link>)}</div></section></>}
      {!!activeProductCategory && <section className="mx-auto w-full max-w-[1500px] pb-9"><div className="px-3 pb-3 pt-5 md:px-6 md:pb-5 md:pt-7"><div className="flex items-end justify-between"><div><div className="mb-1 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#173A2D]" /><span className="font-serif text-[6px] tracking-[0.21em] text-[#9D7B40]">GENAN EDIT</span></div><h1 className="text-[20px] font-semibold tracking-[-0.03em] text-[#173A2D] md:text-[28px]">{activeProductCategory.name_ar}</h1></div><div className="text-left"><span className="block text-[12px] font-semibold leading-none text-[#9D7B40]">{totalProductCount}</span><span className="mt-1 block text-[6px] text-[#929A92]">منتج</span></div></div></div><div className="px-3 md:px-6"><ProductListFilters values={{ query: productQuery, brand: brandFilter, sort: productSort, inStockOnly, minPrice, maxPrice }} brands={availableBrands} resultCount={totalProductCount} onChange={updateProductFilters} /></div><div className="px-2.5 pt-4 md:px-6 md:pt-6">{initialProductsLoading ? <div className="grid grid-cols-2 gap-x-2.5 gap-y-5 sm:gap-x-3 md:grid-cols-3 md:gap-5 lg:grid-cols-4 xl:grid-cols-5">{Array.from({ length: 10 }).map((_, index) => <div key={index}><div className="aspect-[4/5] animate-pulse rounded-[14px] bg-[#EEEAE1]" /><div className="mt-2.5 h-2.5 w-[70%] animate-pulse rounded-full bg-[#E8E3D8]" /><div className="mt-2 h-2.5 w-[36%] animate-pulse rounded-full bg-[#E8E3D8]" /></div>)}</div> : products.length === 0 ? <div className="flex min-h-[42vh] flex-col items-center justify-center px-5 text-center"><div className="flex h-[62px] w-[62px] items-center justify-center rounded-full bg-[#EAE5D7]"><span className="text-[20px] font-light text-[#9D7B40]">F</span></div><h2 className="mt-4 text-[14px] font-semibold text-[#263B31]">لا توجد منتجات حالياً</h2><p className="mt-1.5 max-w-[260px] text-[8px] leading-5 text-[#899289]">جرّبي تغيير الفلاتر أو اختيار ماركة أخرى.</p></div> : <><div className="grid grid-cols-2 gap-x-2.5 gap-y-5 sm:gap-x-3 sm:gap-y-6 md:grid-cols-3 md:gap-x-5 md:gap-y-8 lg:grid-cols-4 xl:grid-cols-5">{products.map((product, index) => <div key={product.id} className="min-w-0"><ProductCard product={product} index={index} /></div>)}</div>{hasMore && <div className="flex flex-col items-center pb-2 pt-10 md:pt-12"><button type="button" onClick={handleLoadMore} disabled={productsLoading} className="flex h-[43px] min-w-[160px] items-center justify-center rounded-full border border-[#D4C9B3] bg-white px-6 text-[9px] font-medium text-[#45584E] transition-colors active:bg-[#F5F1E7] disabled:opacity-50">{productsLoading ? "جاري التحميل..." : "عرض المزيد"}</button><span className="mt-2 text-[7px] text-[#929A92]">{products.length} / {totalProductCount}</span></div>}</>}</div></section>}
    </main><Footer /></div>
  );
};
